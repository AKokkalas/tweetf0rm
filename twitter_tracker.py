...
def collect_tweets_by_search_terms(search_configs_filepath, output_folder, config):
    ...
    for search_config_id in itertools.cycle(search_configs):
        ...
        try:
            no_new_tweets = False
            twitterCralwer = TwitterCrawler(
                apikeys=apikeys, client_args=CLIENT_ARGS, output_folder=output_folder)
            # For windows
            ...
            since_id, no_new_tweets = twitterCralwer.search_by_query(
                querystring, geocode=geocode, since_id=since_id, search_name=output_folder.rsplit('/', 1)[-1])  # added search_name to find it in mongo db
        ...
        flash_cmd_config(
            search_configs, search_configs_filepath, output_folder)
...
def collect_tweets_by_ids(tweet_ids_config_filepath, output_folder, config):
    ...
    current_ix = tweet_ids_config['current_ix'] if (
        'current_ix' in tweet_ids_config) else 0
    total = len(tweet_ids_config['tweet_ids'][current_ix:])
    tweet_id_chuncks = util.chunks(
        tweet_ids_config['tweet_ids'][current_ix:], max_range)
    ...
    for tweet_ids in tweet_id_chuncks:
        try:
            ...
            logger.error(exc)
            # don't care, if Ctrl+c is hit, does not handle it.  When you restart, it restarts from the last chunk (too much trouble to handle Ctrl + c).
            logger.error(util.full_stack())
        ...
        flash_cmd_config(tweet_ids_config,
                         tweet_ids_config_filepath, output_folder)
...
def collect_tweets_by_id_range(tweet_id_range_config_filepath, output_folder, config):
    ...
    max_range = 100
    current_id = tweet_id_range_config['current_id'] if (
        'current_id' in tweet_id_range_config) else 0
    end_id = tweet_id_range_config['end_id'] if (
        'end_id' in tweet_id_range_config) else 0
    ...
    for tweet_ids in tweet_id_chuncks:
        ...
        flash_cmd_config(tweet_id_range_config,
                         tweet_id_range_config_filepath, output_folder)

        logger.info(
            'COMPLETED -> (current_id/end_id: [%d/%d])' % (current_id, end_id))
        logger.info('PAUSE %ds to CONTINUE...' % WAIT_TIME)
        time.sleep(WAIT_TIME)
    else:
...
def collect_users(call, users_config_filepath, output_folder, config):
    ...
    max_range = 100
    current_ix = users_config['current_ix'] if (
        'current_ix' in users_config) else 0
...
def collect_tweets_by_user_ids(users_config_filepath, output_folder, config):
    ...
    for user_config_id in itertools.cycle(users_config):
        ...
        logger.info(
            'REQUEST -> (user_id: [%d]; since_id: [%d])' % (user_id, since_id))
        ...
        try:
            twitterCralwer = TwitterCrawler(
                apikeys=apikeys, client_args=CLIENT_ARGS, output_folder=output_folder)
            since_id, remove = twitterCralwer.fetch_user_timeline(
                user_id, since_id=since_id)
        except Exception as exc:
            logger.error(exc)
            logger.error(util.full_stack())
            pass
        ...
        logger.info(
            'COMPLETED -> (user_id: [%d]; since_id: [%d]; remove: [%s])' % (user_id, since_id, remove))
        logger.info('PAUSE %ds to CONTINUE...' % WAIT_TIME)
        time.sleep(WAIT_TIME)
...
def collect_user_relatinoships_by_user_ids(call, user_ids_config_filepath, output_folder, config):
    ...
    current_ix = user_ids_config['current_ix'] if (
        'current_ix' in user_ids_config) else 0
    ...
    for user_id in user_ids:
        ...
        flash_cmd_config(
            user_ids_config, user_ids_config_filepath, output_folder)
...
def collect_places(call, places_config_filepath, output_folder, config):
    ...
    current_ix = places_config['current_ix'] if (
        'current_ix' in places_config) else 0
    ...
    for place in places:
        try:
            twitterCralwer = TwitterCrawler(
                apikeys=apikeys, oauth2=False, client_args=CLIENT_ARGS, output_folder=output_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-c', '--config', help="config.json that contains twitter api keys;", default="./config.json")
    parser.add_argument(
        '-o', '--output', help="output folder data", default="./data/")
    parser.add_argument(
        '-cmd', '--command', help="commands: search, timeline, tweets_by_ids", default=None)
    parser.add_argument('-cc', '--command_config',
                        help="existing progress data", default="search.json")
    parser.add_argument('-wait', '--wait_time',
                        help="wait time to check available api keys", type=int, default=5)
    ...
    with open(os.path.abspath(args.config), 'r') as config_f:
        ...
        try:
            if (args.command == 'search'):
                collect_tweets_by_search_terms(
                    args.command_config, output_folder, config)
            elif (args.command == 'user_timelines'):
                collect_tweets_by_user_ids(
                    args.command_config, output_folder, config)
            elif (args.command == 'tweets_by_ids'):
                collect_tweets_by_ids(
                    args.command_config, output_folder, config)
            elif (args.command == 'tweets_by_id_range'):
                collect_tweets_by_id_range(
                    args.command_config, args.output, config)
            elif (args.command == 'users_by_ids'):
                collect_users('user_id', args.command_config,
                              args.output, config)
            elif (args.command == 'users_by_screen_names'):
                collect_users('screen_name', args.command_config,
                              args.output, config)
            elif (args.command in ['/friends/ids', '/friends/list', '/followers/ids', '/followers/list']):
                collect_user_relatinoships_by_user_ids(
                    args.command, args.command_config, args.output, config)
            elif (args.command == 'places_by_queries'):
                collect_places('query', args.command_config,
                               args.output, config)
